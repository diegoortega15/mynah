...
export default async function reviewRoutes(app) {
  ...
  // Submit a review for one card.
  app.post('/api/cards/:cardId/review', (req, reply) => {
    ...
    db.prepare(
      'UPDATE cards SET ease = ?, interval_days = ?, reps = ?, state = ?, due_date = ? WHERE id = ?'
    ).run(next.ease, next.interval_days, next.reps, next.state, dueDate, card.id);
